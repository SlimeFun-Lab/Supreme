...
public class TechMutation extends SimpleItemContainerMachine implements Radioactive {
  ...
  public static final SlimefunItemStack TECH_MUTATION_I = new SupremeItemStack("SUPREME_TECH_MUTATION_I",
      ...
      "&3Supreme Machine");
  public static final ItemStack[] RECIPE_TECH_MUTATION_I = {SupremeComponents.INDUCTIVE_MACHINE.asOne(),
      SupremeComponents.SYNTHETIC_RUBY.asOne(), SupremeComponents.INDUCTIVE_MACHINE.asOne(), SlimefunItems.REINFORCED_PLATE.asOne(),
      SlimefunItems.NUCLEAR_REACTOR.asOne(), SlimefunItems.REINFORCED_PLATE.asOne(), SupremeComponents.RUSTLESS_MACHINE.asOne(),
      SupremeCore.CORE_OF_DEATH.asOne(), SupremeComponents.RUSTLESS_MACHINE.asOne()};
  ...
  public static final SlimefunItemStack TECH_MUTATION_II = new SupremeItemStack("SUPREME_TECH_MUTATION_II",
      ...
      LoreBuilder.machine(MachineTier.END_GAME, MachineType.MACHINE), UtilEnergy.energyPowerPerSecond(500), "",
      "&3Supreme Machine");
  public static final ItemStack[] RECIPE_TECH_MUTATION_II = new ItemStack[]{SupremeComponents.CONVEYANCE_MACHINE.asOne(),
      SupremeCetrus.CETRUS_LUMIUM.asOne(), SupremeComponents.CONVEYANCE_MACHINE.asOne(), SupremeComponents.INDUCTOR_MACHINE.asOne(),
      TechMutation.TECH_MUTATION_I.asOne(), SupremeComponents.INDUCTOR_MACHINE.asOne(), SupremeComponents.THORNERITE.asOne(),
  ...
  public static final SlimefunItemStack TECH_MUTATION_III = new SupremeItemStack("SUPREME_TECH_MUTATION_III",
      ...
      LoreBuilder.machine(MachineTier.END_GAME, MachineType.MACHINE), UtilEnergy.energyPowerPerSecond(500), "",
      "&3Supreme Machine");
  public static final ItemStack[] RECIPE_TECH_MUTATION_III = new ItemStack[]{SupremeComponents.THORNERITE.asOne(),
      SupremeAttribute.getImpetus().asOne(), SupremeComponents.THORNERITE.asOne(), SupremeComponents.SUPREME.asOne(),
      TechMutation.TECH_MUTATION_II.asOne(), SupremeComponents.SUPREME.asOne(), SupremeComponents.CRYSTALLIZER_MACHINE.asOne(),
    ...
    if (itemProcessing == null) {
      ...
      if (this.getProgressTime(b) <= 0) {
        ...
        if (UtilMachine.getRandomInt() <= (itemProcessing.getChance() * getUpgradeLuck())) {
          inv.pushItem(((ItemStack) itemProcessing.getOutput().asOne()).clone(), this.getOutputSlots());
        ...
        this.processTicks(b, inv, itemProcessing.getOutput().asOne());
  ...
  private MobTechMutationGeneric validRecipeItem(BlockMenu inv) {
    ...
    for (MobTechMutationGeneric produce : TechMutation.recipes) {
      ItemStack input1 = produce.getInput1().asOne();
      ItemStack input2 = produce.getInput2().asOne();
  ...
  public List<ItemStack> getDisplayRecipes() {
    ...
    TechMutation.recipes
      ...
      displayRecipes.add(recipe.getInput1().asOne());
      displayRecipes.add(CustomItemStack.create(Material.NAME_TAG, " " + chance + "% chance"));
      displayRecipes.add(recipe.getInput2().asOne());
      displayRecipes.add(recipe.getOutput().asOne());
